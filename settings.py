...
TIME_DELAY = [10, 20]                             # Задержка после ТРАНЗАКЦИЙ     [min, max]
TIME_ACCOUNT_DELAY = [10, 15]                     # Задержка между АККАУНТАМИ     [min, max]
...
VALUE_SWAP = [0.00001, 0.0001, 6]                          # [min, max, decimal]
...
routes_shuffle = False                                # Перемешивает модули || True/ False
TIME_DELAY_ROUTES = [10, 12]                       # [min, max] Задержка между модулями
...
QUANTITY_NFT_31 = [1, 2]                          # Количество NFT для минта          [min, max]
NUMBER_TRANS_31 = [3, 6]                          # Количество транзакций             [min, max]
